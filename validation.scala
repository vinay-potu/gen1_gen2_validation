...
def writeToTable(env:String, dag_id:String, gen1_path:String, gen2_path:String, 
                 ...
                 except_validation:String,schemaValRes:String, errMsg:String):Unit=
{
   val fnl_tbl=Seq((env,dag_id, gen1_path, gen2_path, gen1_cnt, 
                    ...
                    dis_count_validation, except_validation, schemaValRes ,errMsg)).
                toDF("env","dag_id", "gen1_path", "gen2_path", "gen1_count", 
                     ...
                     "schema_validation","error_msg")
        //fnl_tbl.show()
        val adlsPath = s"/dbfs/FileStorage/validation_phase2"
        val fnl_db = s"default"
        val rcn_table = s"validation_phase2"
...
def writeToTable_phase4_test(env:String, dag_id:String, gen1_path:String, gen2_path:String, 
                 gen1_cnt:String,gen2_cnt:String,gen1_dis_cnt:String,gen2_dis_cnt:String,
                 Gen1_ex_cnt:String,Gen2_ex_cnt:String,fileValRes:String,
                 count_validation:String,dis_count_validation:String,
                 except_validation:String,schemaValRes:String, errMsg:String):Unit=
{
   val fnl_tbl=Seq((env,dag_id, gen1_path, gen2_path, gen1_cnt, 
                ...
                toDF("env","dag_id", "gen1_path", "gen2_path", "gen1_count", 
                     ...
                     "schema_validation","error_msg")
        //fnl_tbl.show()
        val adlsPath = s"/dbfs/FileStorage/validation_phase4_test"
        val fnl_db = s"default"
        val rcn_table = s"validation_phase4_test"
        fnl_tbl.withColumn("ts",current_timestamp()).write.format("delta").option("mergeSchema", "true").mode("append").option("path",adlsPath).saveAsTable(s"$fnl_db.$rcn_table")
}

def getDependentJobNames(tableList: List[String]): DataFrame=
{
  import spark.implicits._
  val columns = Seq("jobName")
  val data = Seq(("Cloud Migration"))
  val rdd = spark.sparkContext.parallelize(data)
  var df = spark.sparkContext.parallelize(data).toDF
  for(index <- tableList)
  {
    
    df = df.union(spark.sql(s"describe history nzdata.$index").where("job.jobName is not null and job.jobName not in('Cloud Migration','Optimize_nzdata','my spark task')").select("job.jobName").distinct)
  ...
  for(table <- tableList.split("\n") )
  {
    ...
    breakable
    {
        ...
        try{
          ...
          case e: Exception =>{
            ...
            writeToTable(env,dag_id, gen1_path, gen2_path, 
                         "0", "0", "0","0", "-1", "-1", "Failed",
                         "Failed","Failed","Failed","Failed",e.getMessage())
             break
          }
        }
        ...
        writeToTable(env,dag_id, gen1_path, gen2_path, countValRes("gen1_cnt"), 
                    ...
                    exceptValdRes("isPassed"), schemaValRes("isPassed"),schemaValRes("map"))
...
import util.control.Breaks._
import org.apache.spark.sql.DataFrame
def validation_phase4_test(tableList:String): Boolean =
{
  for(table <- tableList.split("\n") )
  {
    if(table.contains(","))
    ...
    breakable
    {
        ...
        try{
          ...
          case e: Exception =>{
            println(s"ERROR:files not present in the path for dag :$dag_id,table:$tableName,ERROR: "+e.getMessage())
            writeToTable_phase4_test(env,dag_id, gen1_path, gen2_path, 
                         "0", "0", "0","0", "-1", "-1", "Failed",
                         "Failed","Failed","Failed","Failed",e.getMessage())
             break
          }
        ...
        val countValRes = countValiadation(gen1DF,gen2DF)
        val countDistValRes = validateDistinctCount(gen1DF,gen2DF, Seq("load_date","load_timestamp"))
        val exceptValdRes = validateData(gen1DF,gen2DF,Seq("load_date","load_timestamp","update_time","load_dtm","update_dtm","azure_export_date","last_update","q_promo_deal_id"))
        writeToTable_phase4_test(env,dag_id, gen1_path, gen2_path, countValRes("gen1_cnt"), 
                    countValRes("gen2_cnt"),countDistValRes("gen1_dis_cnt"), 
                    countDistValRes("gen2_dis_cnt"), exceptValdRes("Gen1_ex_cnt"), 
                    exceptValdRes("Gen2_ex_cnt"),fileValRes, 
                    countValRes("isPassed"), countDistValRes("isPassed"), 
